    ...
    <>
      <Card className="w-full border rounded-lg shadow-md">
        <CardHeader className="flex justify-between items-center">
          <h2 className="text-2xl font-bold">Engellenmiş Zaman Dilimi</h2>
          <Button className="ml-auto" onClick={() => setIsDialogOpen(true)}>
            {isEmpty() ? "Engellenmiş Zaman Dilimini Belirle" : "Engellenmiş Zaman Dilimini Düzenle"}
          </Button>
        </CardHeader>
        <CardContent className="p-4">
          ...
          {hasSlots ? (
                      ...
                      {(dayData.slots ?? []).map((slot, index) => (
                        <div key={index} className="grid grid-cols-3 gap-4 text-sm py-2">
                          ...
                          <span className={slot.isAvailable ? "text-primary" : "text-destructive"}>
                            {slot.isAvailable ? "Uygun" : "Engellenmiş"}
                          </span>
                        </div>
                      ))}
                    </div>
            ...
            <p className="text-center text-muted-foreground">
              Engellenmiş zaman dilimi mevcut değil. Eklemek için `Engellenmiş Zaman Dilimini Belirle` tıklayın.
            </p>
          )}
        </CardContent>
      </Card>
